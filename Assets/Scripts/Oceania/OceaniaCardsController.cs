...
public class OceaniaCardsController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        AddCards();
		cantSelected = 0;
		OceaniaTimeController.enMarcha = true;
    }

    // Update is called once per frame
    void FixedUpdate()
    {
		if(OceaniaTimeController.enMarcha == false){
			OceaniaController.CambiarEscena("Niveles");
		}
        ...
        if(cantSelected == 2){
			...
			OceaniaCardDefinition xScript = XSelected.GetComponent<OceaniaCardDefinition>();
			OceaniaCardDefinition yScript = YSelected.GetComponent<OceaniaCardDefinition>();
	...
	void GenerateCards(){
		...
		for(int i=0;i<cant;i+=2){
			...
			OceaniaCardDefinition cS = card.GetComponent<OceaniaCardDefinition>();
			...
			OceaniaCardDefinition cSPair = cardPair.GetComponent<OceaniaCardDefinition>();
	...
	void SetSprites(){
		spriteReverse = Resources.Load <Sprite>("Rever/Reverso_Oceania");
		Sprite s = Resources.Load <Sprite>("Rever/Reverso_Oceania");
	...
	bool isLevelCompleted(){
		for(int i=0;i<cardsArray.Length;++i){
			if(cardsArray[i].GetComponent<OceaniaCardDefinition>().status != 2)	return false;
		}
		return true;
	}
