...
public class EuropaCardsController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        AddCards();
		cantSelected = 0;
		EuropaTimeController.enMarcha = true;
    }

    // Update is called once per frame
    void FixedUpdate()
    {
		if(EuropaTimeController.enMarcha == false){
        ...
        if(cantSelected == 2){
			...
			EuropaCardDefinition xScript = XSelected.GetComponent<EuropaCardDefinition>();
			EuropaCardDefinition yScript = YSelected.GetComponent<EuropaCardDefinition>();
	...
	void GenerateCards(){
		...
		for(int i=0;i<cant;i+=2){
			...
			EuropaCardDefinition cS = card.GetComponent<EuropaCardDefinition>();
			...
			EuropaCardDefinition cSPair = cardPair.GetComponent<EuropaCardDefinition>();
	...
	void SetSprites(){
		spriteReverse = Resources.Load <Sprite>("Rever/Reverso_Europa");
		Sprite s = Resources.Load <Sprite>("Rever/Reverso_Europa");
	...
	bool isLevelCompleted(){
		for(int i=0;i<cardsArray.Length;++i){
			if(cardsArray[i].GetComponent<EuropaCardDefinition>().status != 2)	return false;
		}
		return true;
	}
