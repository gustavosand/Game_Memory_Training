...
public class AfricaCardsController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        AddCards();
		cantSelected = 0;
		AfricaTimeController.enMarcha = true;
    }

    // Update is called once per frame
    void FixedUpdate()
    {
		if(AfricaTimeController.enMarcha == false){
        ...
        if(cantSelected == 2){
			...
			AfricaCardDefinition xScript = XSelected.GetComponent<AfricaCardDefinition>();
			AfricaCardDefinition yScript = YSelected.GetComponent<AfricaCardDefinition>();
	...
	void GenerateCards(){
		...
		for(int i=0;i<cant;i+=2){
			...
			AfricaCardDefinition cS = card.GetComponent<AfricaCardDefinition>();
			...
			AfricaCardDefinition cSPair = cardPair.GetComponent<AfricaCardDefinition>();
	...
	void SetSprites(){
		spriteReverse = Resources.Load <Sprite>("Rever/Reverso_Africa");
		Sprite s = Resources.Load <Sprite>("Rever/Reverso_Africa");
	...
	bool isLevelCompleted(){
		for(int i=0;i<cardsArray.Length;++i){
			if(cardsArray[i].GetComponent<AfricaCardDefinition>().status != 2)	return false;
		}
		return true;
	}
